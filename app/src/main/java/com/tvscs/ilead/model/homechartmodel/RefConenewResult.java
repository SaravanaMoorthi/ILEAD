package com.tvscs.ilead.model.homechartmodel;

import java.io.Serializable;

public class RefConenewResult implements Serializable {
    private String Ctwo;

    private String Cfour;

    private String Product;

    private String Productdesc;

    private String Cone;

    private String Czero;

    private String Cthree;

    public String getCtwo ()
    {
        return Ctwo;
    }

    public void setCtwo (String Ctwo)
    {
        this.Ctwo = Ctwo;
    }

    public String getCfour ()
    {
        return Cfour;
    }

    public void setCfour (String Cfour)
    {
        this.Cfour = Cfour;
    }

    public String getProduct ()
    {
        return Product;
    }

    public void setProduct (String Product)
    {
        this.Product = Product;
    }

    public String getProductdesc ()
    {
        return Productdesc;
    }

    public void setProductdesc (String Productdesc)
    {
        this.Productdesc = Productdesc;
    }

    public String getCone ()
    {
        return Cone;
    }

    public void setCone (String Cone)
    {
        this.Cone = Cone;
    }

    public String getCzero ()
    {
        return Czero;
    }

    public void setCzero (String Czero)
    {
        this.Czero = Czero;
    }

    public String getCthree ()
    {
        return Cthree;
    }

    public void setCthree (String Cthree)
    {
        this.Cthree = Cthree;
    }

    @Override
    public String toString()
    {
        return "ClassPojo [Ctwo = "+Ctwo+", Cfour = "+Cfour+", Product = "+Product+", Productdesc = "+Productdesc+", Cone = "+Cone+", Czero = "+Czero+", Cthree = "+Cthree+"]";
    }
}

